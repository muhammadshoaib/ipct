...
window.onload = function() {
	...
	.click(
			function(event) {
				...
				var center_height = $(window).scrollTop() + $(window).height() / 2
				...
				var simulation = d3.forceSimulation()
				    ...
				    //.force("linkDistance", d3.forceLinkDistance( 40))
				    .force("center", d3.forceCenter( centerX, centerY));


				d3.json("GeneSearch?" + idata, function(json) {
					alert(JSON.stringify(json));

					$("#cygraph").html(""); 
					var svg = d3.select("#cygraph").append("svg")
					.attr("width", '100%')
					.attr("height", '100%')
